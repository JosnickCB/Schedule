<!doctype html>
<html lang=''>
<head>
   <meta charset='utf-8'>
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1">
   <link rel="stylesheet" href="styles.css">
   <script src="http://code.jquery.com/jquery-latest.min.js" type="text/javascript"></script>
   <title>Primero</title>
</head>
<body>

<div id='cssmenu'>
<ul>
   <li class='active'><a href='index.php'>Home</a></li>
   <li><a href='primero.php'>Primero</a></li>
   <li><a href='segundo.php'>Segundo</a></li>
   <li><a href='tercero.php'>Tercero</a></li>
   <li><a href='cuarto.php'>Cuarto</a></li>
   <li><a href='quinto.php'>Quinto</a></li>
   <li><a href='login.php'>Ingresar</a></li>
</ul>
</div>
<table class = flat-table>
    <tr>
      <th></th>
      <th>Lunes</th>
      <th>Martes</th>
      <th>Miercoles</th>
      <th>Jueves</th>
      <th>Viernes</th>
    </tr>
    <tr>
      <td>7.00 - 7.50</td>
      <td></td>
      <td>Física Computacional teo (B)</td>
      <td>Física Computacional lab (B)</td>
      <td>Proyecto de Final de Carrera I teo(A)</td>
      <td>Computación Paralela y Distribuida prac(A)</td>
    </tr>
    <tr>
      <td>7.50 - 8.40</td>
      <td>Iteración Humano Computador teo (A)</td>
      <td>Física Computacional prac (B)</td>
      <td>Física Computacional lab (B)</td>
      <td>Proyecto de Final de Carrera I teo(A)</td>
      <td>Computación Paralela y Distribuida prac(A)</td>
    </tr>

    <tr>
      <td>8.40 - 8.50</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    </tr>

    <tr>
      <td>8.50 - 9.40</td>
      <td>Seguridad en Computación teo</td>
      <td>Física Computacional prac (B)</td>
      <td>Formación de Empresas de Base Tecnológica I teo</td>
      <td>Computación Paralela y Distribuida lab(B)</td>
      <td>Computación Paralela y Distribuida pract(B)</td>
    </tr>
    <tr>
      <td>9.40 - 10.30</td>
      <td>Seguridad en Computación teo</td>
      <td></td>
      <td>Formación de Empresas de Base Tecnológica I teo</td>
      <td>Computación Paralela y Distribuida lab(B)</td>
      <td>Computación Paralela y Distribuida pract(B)</td>
    </tr>   

    <tr>
      <td>10.30 - 10.40</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    </tr>

    <tr>
      <td>10.40 - 11.30</td>
      <td>Seguridad en Computación prac</td>
      <td></td>
      <td>Seguridad en Computación lab(A)</td>
      <td>Proyecto Final de Carrera I teo(B)</td>
      <td>Formación de Empresas de Base Tecnológica I prac(B)</td>
    </tr>
    <tr>
      <td>11.30 - 12.20</td>
      <td>Tópicos en Ingenieria de Software teo</td>
      <td></td>
      <td>Seguridad en Computación lab(A)</td>
      <td>Proyecto Final de Carrera I teo(B)</td>
      <td>Formación de Empresas de Base Tecnológica I prac(B)</td>
    </tr>
    <tr>
      <td>12.20 - 13.10</td>
      <td>Tópicos en Ingenieria de Software teo</td>
      <td>Lenguajes de programación lab</td>
      <td>Seguridad en Computación lab(B)</td>
      <td>Tópicos en Ingenieria de Software prac(A)</td>
      <td>Iteracción Humano Computador prac(A)</td>
    </tr>
    <tr>
      <td>13.10 - 14.00</td>
      <td>Física Computacional teo(A)</td>
      <td>Lenguajes de programación lab</td>
      <td>Seguridad en Computación lab(B)</td>
      <td>Tópicos en Ingenieria de Software prac(A)</td>
      <td>Iteracción Humano Computador prac(A)</td>
    </tr>
    <tr>
      <td>14.00 - 14.50</td>
      <td>Física Computacional prac(A)</td>
      <td>Ética profesional teo</td>
      <td>Computación Paralela y Distribuida teo(A)</td>
      <td>Física Computacional lab(A)</td>
      <td>Iteracción Humano Computador prac(A)</td>
    </tr>
    <tr>
      <td>14.50 - 15.40</td>
      <<td>Física Computacional prac(A)</td>
      <td>Ética profesional teo</td>
      <td>Computación Paralela y Distribuida teo(A)</td>
      <td>Física Computacional lab(A)</td>
      <td>Iteracción Humano Computador prac(A)</td>
    </tr>

      <td>15.40 - 15.50</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    </tr>

    <tr>
      <td>15.50 - 16.40</td>
      <td>Lenguajes de Programación teo</td>
      <td>Programación de dispositivos mob teo</td>
      <td></td>
      <td>Proyecto Final de Carrera I prac(A)</td>
      <td>Tópicos en Ingenieria de Software prac(B)</td>
    </tr>
    <tr>
      <td>16.40 - 17.30</td>
      <td>Lenguajes de Programación teo</td>
      <td>Programación de dispositivos mob teo</td>
      <td></td>
      <td>Proyecto Final de Carrera I prac(A)</td>
      <td>Tópicos en Ingenieria de Software prac(B)</td>
    </tr>

    <tr>
      <td>17.30 - 17.40</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    </tr>

    <tr>
      <td>17.40 - 18.30</td>
      <td>Computación Paralela y Distruibuida teo(B)</td>
      <td>Computación Paralela y Distruibuida lab(A)</td>
      <td>Relaciones Humanas teo</td>
      <td>Seminario de Tesis teo</td>
      <td>Proyecto Final de Carrera I prac(B)</td>
    </tr>
    <tr>
      <td>18.30 - 19.20</td>
      <td>Computación Paralela y Distruibuida teo(B)</td>
      <td>Computación Paralela y Distruibuida lab(A)</td>
      <td>Relaciones Humanas teo</td>
      <td>Seminario de Tesis teo</td>
      <td>Proyecto Final de Carrera I prac(B)</td>
    </tr>

    <tr>
      <td>19.20 - 19.30</td>
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    </tr>

    <tr>
      <td>19.30 - 20.10</td>
      <td>Seguridad Computacional prac</td>
      <td>Lenguajes de Programación</td>
      <td>Relaciones Humanas teo</td>
      <td></td>
      <td></td>
    </tr>
	</table>
</body>
<html>